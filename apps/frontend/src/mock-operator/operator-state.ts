...
export function createOperatorFixtureState(): OperatorFixtureState {
  return {
    ...
    systemStatus: {
      service: "api",
      status: "ok",
      environment: "manual-walkthrough",
...
export function createValidationResult(symbolCode: string): SymbolValidationResponse {
  return {
    ...
    provider: "manual_walkthrough_mt5",
    is_valid: true,
    reason: null,
...
export function setPaperTradingRuntime(
  ...
  state.tradingStatus = {
    ...
    last_started_by: enabled ? "manual-walkthrough-operator" : state.tradingStatus.last_started_by,
    last_stopped_at: enabled ? state.tradingStatus.last_stopped_at : timestamp,
    last_stopped_by: enabled ? state.tradingStatus.last_stopped_by : "manual-walkthrough-operator",
  };
